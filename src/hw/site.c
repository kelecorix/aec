...
void run(Site* site) {
  ...
  site->logger->eventLOG->fp = fopen(site->logger->eventLOG->filename, "a");
...
int site_mode_uvo(Site* site) {
  ...
  fprintf(site->logger->eventLOG->fp, "Перед while UVO");
  while (1) {
      ...
      if ((time(NULL) - site->vents[0]->time_start) > 30) {
        if (site->tacho1_exists && site->tacho2_exists) {
          ...
          fprintf(site->logger->eventLOG->fp,
              ...
              site->tacho1, site->tacho1_t, site->vents[1]->step, site->tacho2,
              site->tacho2_t);
...
void sub_uvo_th(Site* site, int fail) {
  ...
  fprintf(site->logger->eventLOG->fp,
      "sub_uvo_th temp_dew = %f site->temp_out = %f \n", temp_dew,
      site->temp_out);
  if (site->temp_out > temp_dew) {
    fprintf(site->logger->eventLOG->fp, "Где заслонка\n");
    // Заслонка выставлена на улицу
    if (site->th->position != 10) {

      fprintf(site->logger->eventLOG->fp, "Переводим заслонку на улицу\n");
    ...
    if ((time(NULL) - site->th->time_start) >= 120) {
      ...
      fprintf(site->logger->eventLOG->fp, "Да настало, а есть ли откуда читать?\n");
      site->th->time_start = time(NULL);
      if (site->th_r_exists) {
        ...
        if (site->th->position == site->th_r) {
          ...
          fprintf(site->logger->eventLOG->fp, "Авария заслонки\n");
          if (site->vents[0]->mode == 1 || site->vents[1]->mode == 1) {
            for (v = 0; v < 2; v++) {
              site->vents[v]->set_step(site->vents[v], 0);
            }
    ...
    if (site->temp_mix >= temp_dew) {
      ...
      if (curr_pos != 10) {
        ...
        fprintf(site->logger->eventLOG->fp, "Приоткроем заслонку %d\n", curr_pos);
        site->th->time_start = time(NULL);
      }
    } else {
      ...
      if (curr_pos != 0) {
        ...
        fprintf(site->logger->eventLOG->fp, "Призакроем заслонку %d\n", curr_pos);
        site->th->time_start = time(NULL);
      }
    }
  }
...
int sub_uvo_fail(Site* site) {
  ...
  fprintf(site->logger->eventLOG->fp, "sub_uvo_fail\n");
  ...
  if ((site->temp_in) >= temp_fail) {
    ...
    fprintf(site->logger->eventLOG->fp,
        "sub_uvo_fail site->temp_in = %f temp_support = %f\n", site->temp_in,
        temp_support);
    ...
    if ((site->temp_in >= (temp_support - 2))
      ...
      fprintf(site->logger->eventLOG->fp, "Перейдем sub_uvo_vent\n");
      //sub_uvo_vent(site);
      // передадим исполнение в основную ветку
      return EXIT_FAILURE;
    } else {

      fprintf(site->logger->eventLOG->fp, "Температура не в пределах\n");
      fprintf(site->logger->eventLOG->fp, "Перейдем sub_uvo_pen\n");
...
int site_mode_ac(Site* site) {
  ...
  for (a = 0; a < 2; a++) {
    ...
    fprintf(site->logger->eventLOG->fp,
        "Включили кондиционер КОНД_%d время включения %d\n", a,
        site->acs[a]->time_start);
  }

  fprintf(site->logger->eventLOG->fp, "До while Режим охлаждения кондиционером\n");
    ...
    if (difftime(time(NULL), site->time_pre) <= 5) { //секунды
      ...
      for (a_cond = 0; a_cond < num_ac; a_cond++) { //TODO количество кондиционеров брать из конфига
        if (((site->temp_in - site->acs[a_cond]->temp) >= 10)
          ...
          fprintf(site->logger->eventLOG->fp,
              "Набрал дельту КОНД_%d %f дельта %f\n", a_cond,
              site->acs[a_cond]->temp,
              (site->temp_in - site->acs[a_cond]->temp));
          if (site->vents[0]->mode == 1 || site->vents[1]->mode == 1) {
            // да

            fprintf(site->logger->eventLOG->fp, "выключим вентиляцию\n");
            //выключим вентиляцию
            for (v = 0; v < 2; v++) {
        ...
        fprintf(site->logger->eventLOG->fp, "Цикл КОНД_%d\n", a_cond);
        ...
        && (site->acs[a_cond]->mode == 1)) {
          ...
          fprintf(site->logger->eventLOG->fp,
              ...
              time(
              NULL), site->acs[a_cond]->time_start,
              (difftime(time(NULL), site->acs[a_cond]->time_start)),
              site->acs[a_cond]->is_diff);
          if (site->acs[a_cond]->is_diff == 0) {

            fprintf(site->logger->eventLOG->fp,
                ("Авария кондиционера num_ac_tmp = %d\n", num_ac_tmp));
            // Авария кондиционера
            num_ac_tmp--;

            fprintf(site->logger->eventLOG->fp, ("num_ac_tmp = %d", num_ac_tmp));
            // Выключим нерабочий кондиционер

            fprintf(site->logger->eventLOG->fp,
                ("Выключим нерабочий кондиционер КОНД_%d\n", a_cond));
      ...
      fprintf(site->logger->eventLOG->fp,
          ("еще есть живые кондиционеры? num_ac_tmp = %d\n", num_ac_tmp));
      // еще есть живые кондиционеры?
      if (num_ac_tmp > 0) {

        fprintf(site->logger->eventLOG->fp,
            ("Да еще есть кондиционеры num_ac_tmp = %d\n", num_ac_tmp));
        //да
        float temp_support = strtof(getStr(site->cfg, (void *) "temp_support"),
        NULL);
        ...
        if ((temp_support - site->temp_out) > (site->penalty + 2)) {
          ...
          if (site->temp_in >= temp_fail) {
            ...
            fprintf(site->logger->eventLOG->fp, ("работа нормальная\n"));
            // работа нормальная
            continue;
          }
        }
        ...
        fprintf(site->logger->eventLOG->fp,
            ("НЕТ живых кондиционеров переходим в аварийный режим\n"));
...
int site_mode_heat(Site* site) {
  ...
  fprintf(site->logger->eventLOG->fp, ("Режим догрева сайта!\n"));
    ...
    while (i < 300) {
      ...
      if (((time(NULL) - site->th->time_start) >= 30) && site->th_r_exists) {
        if (difftime(time(NULL), site->th->time_start) >= 30) {
          ...
          fprintf(site->logger->eventLOG->fp,
              ("site->th->position = %d th_pos_read = %d", site->th->position, th_pos_read));
          if (site->th->position == th_pos_read) {

            fprintf(site->logger->eventLOG->fp, ("да заслонка исправна\n"));
            break;
          } else {

            fprintf(site->logger->eventLOG->fp, ("заслонка не исправна\n"));
            site_mode_fail_uvo(site);
          }
        }
        i++;
  ...
  fprintf(site->logger->eventLOG->fp,
      "Перед ЦИКЛ ДОГРЕВ time %d site->time_pre %d diff %f\n", time(NULL),
    ...
    if (difftime(time(NULL), site->time_pre) <= 2) //30
      ...
      if (site->temp_in >= temp_support - 4) { //TODO: необходимо ввести параметр до какой температуры греть
        ...
        if (site->temp_mix > 60) {
          ...
          fprintf(site->logger->eventLOG->fp,
              ("ТЭН не остыл еще продолжим вентиляцию\n"));
          continue; // продолжаем цикл догрева
        } else {

          fprintf(site->logger->eventLOG->fp, ("ТЭН остыл выключим вентиляцию\n"));
          ...
          fprintf(site->logger->eventLOG->fp,
              ("Перейдем в режим охлаждения УВО\n"));
          site_mode_uvo(site); // режим охлаждения уво
        }
      } else {
        //нет
        fprintf(site->logger->eventLOG->fp, ("Нужно греть сайт\n"));
        // TODO: Проверить по описанию
        ...
        if (difftime(time(NULL), site->vents[0]) > 30) {
          ...
          fprintf(site->logger->eventLOG->fp,
              ("Проверим вращается ли вентилятор\n"));
          //да

          if ((site->vents[0]->step == site->tacho1)
              || (site->tacho1_exists != 1)) {
            fprintf(site->logger->eventLOG->fp, ("Да вращается\n"));
            //да
            site->vents[0]->time_start = time(NULL);
            if ((site->ten == 1) && site->vents[0]->error == ERROR_HEAT) {

              fprintf(site->logger->eventLOG->fp,
                  ("Пока неизвестно вращается ли вентилятор\n"));
              //да
              continue;
              ...
              fprintf(site->logger->eventLOG->fp, ("Да вращается включим ТЭН\n"));
              set_ten(site, 1);
              continue;
            }
          } else {
...
int site_mode_fail_uvo(Site* site) {
  ...
  fprintf(site->logger->eventLOG->fp, ("site_mode_fail_uvo: Режим авария УВО!\n"));
  //    "Режим авария УВО");
  site->mode = 4;
  site->time_pre = time(NULL);
  ...
  while (1) { // sensors was read - ok
    ...
    if (difftime(time(NULL), site->time_pre) <= 5) { //секунды
      ...
      if ((temp_support - site->temp_out) >= (site->penalty + 2)) {
        ...
        if (site->temp_in < temp_support - 2) {
          // переходим в УВО
          fprintf(site->logger->eventLOG->fp, ("Да сайт позволяет\n"));
          site_mode_uvo(site);
        } else {

          fprintf(site->logger->eventLOG->fp, ("Нет сайт не позволяет\n"));
...
int site_mode_fail_ac(Site* site) {
  fprintf(site->logger->eventLOG->fp, ("Режим авария кондиционеров!\n"));
  ...
  while (1) {
    ...
    if (difftime(time(NULL), site->time_pre) <= 30) { //секунды
      ...
      fprintf(site->logger->eventLOG->fp,
          ("site_mode_fail_ac: включим вентиляцию\n"));
      // включим вентиляцию
      ...
      for (a = 0; a < 2; a++) {
        if (site->acs[a]->error == NOPOWER) {
          if (site->power == 1) {
            ...
            fprintf(site->logger->eventLOG->fp, ("Да питание есть перейдем\n"));
      ...
      fprintf(site->logger->eventLOG->fp,
          ("Проверим температура не ниже поддержания - 2\n"));
      if (site->temp_in < temp_support - 2) {

        fprintf(site->logger->eventLOG->fp,
            ("Да ниже выключим вениляцию site->vents[v]->mode = %d\n", site->vents[v]->mode));
        // выключим вениляцию
        for (v = 0; v < 2; v++) {
          if (site->vents[v]->mode == 1) {
            site->vents[v]->set_step(site->vents[v], 0);
        ...
        fprintf(site->logger->eventLOG->fp,
            "Сайт не замерз? site->temp_in = %f temp_heat = %f\n", site->temp_in, temp_heat);
        if (site->temp_in < temp_heat) {

          fprintf(site->logger->eventLOG->fp, ("Да замерз пойдем греть\n"));
          site_mode_heat(site);
        }
      ...
      fprintf(site->logger->eventLOG->fp,
          ("Переход на функцию логики управления заслонкой\n"));
...
int site_mode_fail_temp(Site* site) {
  ...
  fprintf(site->logger->eventLOG->fp, ("Общий аварийный режим!\n"));
...
int site_mode_fail_temp_uvo(Site* site) {
  ...
  if ((site->temp_out) > temp_dew) {
    if (site->th->exist) //Это есть ли заслонка? или есть ли откуда читать
    {
      ...
      fprintf(site->logger->eventLOG->fp,
          ("переведем заслонку в положение улица\n"));
      site->th->set_position(site->th, 10); //переведем заслонку в положение улица
      site->th->time_start = time(NULL);
    }
  } else {
    if (site->th->exist) {
      ...
      fprintf(site->logger->eventLOG->fp,
          ("переведем заслонку в положение сайт\n"));
      site->th->set_position(site->th, 0); //переведем заслонку в положение сайт
      site->th->time_start = time(NULL);
    }
  }
...
int site_mode_fail_temp_ac(Site* site) {
  ...
  fprintf(site->logger->eventLOG->fp,
      ("Авария по температуре: режим охлаждения кондиционером!\n"));
  ...
  while (1) {
    ...
    if (difftime(time(NULL), site->time_pre) <= 30) { //секунды
      ...
      for (a_cond = 0; a_cond < num_ac; a_cond++) { //TODO количество кондиционеров брать из конфига
        ...
        fprintf(site->logger->eventLOG->fp,
            "600 отработан промежуток? time_start_%d %d time %d diff %d %f\n", a_cond, site->acs[a_cond]->time_start, time(
            NULL), time(NULL) - site->acs[a_cond]->time_start, difftime(
        ...
        && (site->acs[a_cond]->mode == 1)) {
          ...
          fprintf(site->logger->eventLOG->fp,
              ("ДА 600 прошло дельта набрана? time %d - time_start %d = %f is_diff = %d\n", time(
              NULL), site->acs[a_cond]->time_start, (difftime(time(NULL),
...
int set_ten(Site* site, int val) {
  ...
  fprintf(site->logger->eventLOG->fp, ("Изменим тен\n"));
  char buf[1];
  ...
  if (read(g_i2cFile, buf, 1) != 1) {
    fprintf(site->logger->eventLOG->fp, "Error reading from i2c\n");
  }
